   ...
   (let ((ans) (ek) (ex)
    ...
    (dolist (ek (cdr e))
        ...
        (cond ((or (eql ans 0) (eq ans '$zerob) (eq ans '$zeroa))
                (push (list ek ans) zeroterms))

              ((or (eq ans '$minf) (eq ans '$inf) (eq ans '$infinity))
                 (push (list ek ans) infterms))

              ((eq ans '$ind)
                (push (list ek ans) indterms))

              ((eq ans '$und)
                (push (list ek ans) undterms))

              (t (push (list ek ans) otherterms))))
 
   ;; When there are both zeroterms and infterms (an indeterminate form),
	 ;; dispatch limit2 on the indeterminate form.
	 (when (and zeroterms infterms)
	    (setq ek (fapply 'mtimes (mapcar #'car (append infterms zeroterms))))
		;; Likely we should have some logic about which is the numerator?
	    (setq ans (limit2 (fapply 'mtimes (mapcar #'car zeroterms))
                          (div 1 (fapply 'mtimes (mapcar #'car infterms))) x pt))

		  (cond ((or (eq ans '$minf) (eq ans '$inf) (eq ans '$infinity))
                 (setq infterms (list (list ek ans)))
				 (setq zeroterms nil))

			  ((or (eql ans 0) (eq ans '$zerob) (eq ans '$zeroa))
                (setq zeroterms (list (list ek ans)))
				(setq infterms nil))

			  ((eq ans '$und)
			     (push (list ek '$und) undterms)
				 (setq infterms nil)
				 (setq zeroterms nil))

			  (t 
			    (push (list ek ans) otherterms)
			  	(setq infterms nil)
				  (setq zeroterms nil))))
	    ;; We no longer have *both* infterms and zeroterms.				
      (cond 
	     ;; At least one zero term, return a zero. This includes 0 x und
	     (zeroterms 
         (setq ans (reduce #'mult-extended-real (mapcar #'cadr zeroterms)))
         (infsimp (mul ans (fapply 'mtimes (mapcar #'cadr otherterms)))))
		   ;; When the limit of one or more terms is und, return und.
	     (undterms '$und)
		    ;; infterms, but no ind terms, no zero terms, no und terms
		   ((and infterms (not indterms))
         (setq ans (fapply 'mtimes (mapcar #'cadr otherterms)))
         (mult-expr-infinities ans (mapcar #'cadr infterms)))

         ;; infterms & ind terms, no zero terms, no und terms
		   ((and infterms indterms)
		      (setq ex (fapply 'mtimes (mapcar #'car indterms)))
		     (let ((sgn (ftake '%signum ex)))
		  	(if (or (eql sgn -1) (eql sgn '1))
			  (infsimp (cons '(mtimes) 
			    (append (cons sgn (mapcar #'cadr otherterms)) 
